...
```php
updateUser($app_id, $alias_label, $alias_id, $update_user_request, $subscription_id): \onesignal\client\model\InlineResponse202
```
...
try {
    $result = $apiInstance->updateUser($app_id, $alias_label, $alias_id, $update_user_request, $subscription_id);
    print_r($result);
} catch (Exception $e) {
